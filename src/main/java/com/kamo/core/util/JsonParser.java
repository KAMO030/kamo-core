...
public class JsonParser {
    ...
    private static String parseTypeStrategy(Class type, Object object) {
        if (type.isArray()) {
            return array2JSON((Object[]) object);
        ...
        } else if (ReflectUtils.isPrimitive(type)) {
    ...
    private static String do2JSONParser(Class type, Object object) {
        ...
        ReflectUtils.forEachField(type, field -> {
            ...
            if (!ReflectUtils.isPrimitive(fieldValue.getClass())) {
    ...
    private static String array2JSON(Object[] array) {
        ...
        StringJoiner joiner = new StringJoiner(",", "[", "]");
        ReflectUtils.forEachArray(array, a -> joiner.add(object2JSON(a)));
        return joiner.toString();
    }
    ...
    public static <T> T json2Object(String json, Class<T> type, Class... genericsType) {
        ...
        if (json.startsWith("{") && json.endsWith("}")) {
            ...
            if (type.isArray()) {
                return (T) ReflectUtils.collection2Array(setPropertiesByList(valueList, type.getComponentType()));
            } else if (genericsType != null && genericsType.length >= 1 && ClassUtils.isCollection(type)) {
    ...
    public static <T> Map<String, Object> json2Map(String json) {
        json = json.trim();
        if (json.startsWith("{") && json.endsWith("}")) {
            Map<String, Object> valueMap = getKeyValueMap(json);
            return valueMap;
        }
        throw new IllegalArgumentException(json + " json格式错误");
    }
    ...
    private static <T> T setPropertiesByMap(Map<String, Object> keyValueMap, Class<T> type) {
        T object = ReflectUtils.newInstance(type);
        for (String key : keyValueMap.keySet()) {
            Field field = ReflectUtils.getField(type, key, true);
            if (Modifier.isTransient(field.getModifiers())) {
                continue;
            }
            String fieldName = field.getName();
            ...
            if (value instanceof Map) {
                ...
                ReflectUtils.collection2Array(value, values);

            } else if (value instanceof List) {
                value = setPropertiesByList((List) value,(Class) ReflectUtils.getActualTypeFirstArgument(fieldType));
            } else if (value instanceof String) {
                value = ReflectUtils.parseString((Class) fieldType, (String) value);
            } else {
                continue;
            }
    ...
    private static <T> List<T> setPropertiesByList(List<Object> valueString, Class<T> type) {
        ...
        for (Object value : valueString) {
            if (value instanceof Map) {
                ...
                valueList.add(ReflectUtils.parseString(type, (String) value));
            }
        }
        return valueList;
    }
    ...
    private static int getIndex(char ch, String json) {
        int index = 1;
        int spaceCount = 0;
        if (ch == '{' || ch == '[') {
            int count = 1;
    ...
    private static Object getValue(Character ch, String json) {
        if (ch.equals('{')) {
